...
// always let A size small than B, the Kth, k is start from 1 to n, so remeber to use k - 1 as the index
public double findMedianSortedArrays(int A[], int B[]) {
...
private int findKth(int A[], int A_start, int B[], int B_start, int k) {
    ...
    if (k == 1)
        return Math.min(A[A_start], B[B_start]);
    int A_key = (A_start + k/2 - 1) < A.length ? A[A_start + k/2 - 1] : Integer.MAX_VALUE;
    int B_key = B[B_start + k/2 - 1];
    if (A_key < B_key)
        // not A_start + k/2 - 1, because we need to move on
        return findKth(A, A_start + k/2, B, B_start, k - k/2);
    else
        return findKth(A, A_start, B, B_start + k/2, k - k/2);
}
